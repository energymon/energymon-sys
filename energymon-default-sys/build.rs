...
fn main() {
    let lib = "energymon-default";
    env::set_var("ENERGYMON_DEFAULT_STATIC", "");
    match pkg_config::find_library(&lib) {
        ...
        Err(_) => {
            ...
            run(Command::new("cmake").arg(default_impl).arg("-DBUILD_SHARED_LIBS=false").arg(cmake_var).arg(&cmake_gen)
